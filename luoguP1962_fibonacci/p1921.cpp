#include <iostream>
#define P 1000000007

void matrix_mul(long long a[2][2], long long b[2][2], long long ans[2][2]){
    ans[0][0]=(a[0][0]*b[0][0]%P+a[0][1]*b[1][0]%P)%P;
    ans[0][1]=(a[0][0]*b[0][1]%P+a[0][1]*b[1][1]%P)%P;
    ans[1][0]=(a[1][0]*b[0][0]%P+a[1][1]*b[1][0]%P)%P;
    ans[1][1]=(a[1][0]*b[0][1]%P+a[1][1]*b[1][1]%P)%P;
    return;
}

void matrix_pow(long long m[2][2], long long n, long long ans[2][2]){
    ans[0][0]=1; ans[0][1]=0; ans[1][0]=0; ans[1][1]=1;
    long long temp[2][2];
    while (n){
        for (int i=0; i<2; i++){
            for (int j=0; j<2; j++){
                temp[i][j]=ans[i][j];
            }
        }
        if (n%2) matrix_mul(temp, m, ans);
    
        matrix_mul(m,m,temp);
        for (int i=0; i<2; i++){
            for (int j=0; j<2; j++){
                m[i][j]=temp[i][j];
            }
        }
        n/=2;
    }
    return;
}

int main(){
    long long n;
    long long a[2][2]={{1,1},{1,0}};
    long long ans[2][2];
    scanf("%lld", &n);
    if (n==1 || n==2) {
        printf("%d",1);
        return 0;
    }
    matrix_pow(a,n-2,ans);
    //printf("%lld %lld", ans[0][0], ans[0][1]);
    printf("%lld", (ans[0][0]+ans[0][1])%P);
    return 0;
}